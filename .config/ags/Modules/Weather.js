    ...
    (self) => {
        ...
        for (const city of cities) {
            ...
            listStore.set(
                ...
                [0, 1],
                [
                    city.name.toString() + ", " + city.country_code.toString(),
                    city,
                ]
            );
        }
...
async function getWeather() {
    ...
    try {
        // await is needed to wait for the return of the data
        const data = await Utils.fetch(url).then((res) => res.json());
        //.catch(console.error)
        return data;
    } catch {
        // If request fails
...
export const hiTemp = Utils.derive([weather], (weather) => {
    if (weather != null) {
        return (
            "hi: " +
            Math.round(weather.daily.temperature_2m_max[0]).toString() +
            weather.current_units.temperature_2m.toString()
        );
    } else {
        return "0";
...
export const loTemp = Utils.derive([weather], (weather) => {
    if (weather != null) {
        return (
            "lo: " +
            Math.round(weather.daily.temperature_2m_min[0]).toString() +
            weather.current_units.temperature_2m.toString()
        );
    } else {
        return "0";
...
export const weatherImage = Utils.derive([weather], (weather) => {
    if (weather != null) {
        print(
            "weather image = " +
                LookupWeatherCode(weather.current.weather_code).image
        );
...
export const humidity = Utils.derive([weather], (weather) => {
    if (weather != null) {
        return (
            Math.round(weather.current.relative_humidity_2m).toString() + "%"
        );
    } else {
        return "0";
    }
